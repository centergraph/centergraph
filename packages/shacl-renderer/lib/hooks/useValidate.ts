...
const reportSignal = signal<any>(null)
let validate: undefined | (() => void) = undefined

const getErrorMessages = (reportSignal: Signal<any>, path: any) => {
...
export const useValidate = (settings: Settings) => {
  if (!validate) {
    ...
    validate = () => {
      validator.validate({ dataset: settings.dataStore }).then((report: any) => {
